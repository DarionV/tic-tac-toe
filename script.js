const gameBoard = (function () {
  ...
  const getGameBoardContainer = () =>
    document.querySelector(".js-gameboard-container");
  ...
  const resetBoardWithWaveAnimation = function (speed = 100) {
    ...
    for (let i = 0; i < getAllTiles().length; i++) {
      setTimeout(() => {
        if (getAllTiles()[i].getValue() !== "")
          getAllTiles()[i].setValueWithFlip("");
  ...
  const renderMessage = function (messageArray, shouldFlip) {
    ...
    for (let row = 0; row < gameBoard.getGameBoard().length; row++) {
      for (let i = 0; i < gameBoard.getGameBoard().length; i++) {
        if (shouldFlip) {
          if (
            gameBoard.getGameBoard()[row][i].getValue() !==
              messageArray[index] ||
            gameBoard.getGameBoard()[row][i].getToken() !== messageArray[index]
          ) {
            gameBoard
              .getGameBoard()
              [row][i].setValueWithFlip(messageArray[index]);
...
function createComputer(token) {
  ...
  const scanDiagonals = (board, func) => {
    ...
    for (let i = 0; i < 2; i++) {
      ...
      gameBoard.getDiagonals()[i].forEach((tile) => {
        func(tile);
      });
      tokensFound = 0;
      firstToken = "";
